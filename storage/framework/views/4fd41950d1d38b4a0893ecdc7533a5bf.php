...
<?php $__env->startSection('title', 'Edit Retur '.$retur->kd_retur.' - Warehouse Tirta Sago'); ?>
<?php $__env->startSection('breadcrumb', 'Edit Retur Barang'); ?>
...
<div class="flex flex-col w-full pb-container-padding gap-stack-md">
    ...
    <nav class="flex items-center gap-stack-sm font-sidebar-nav text-sidebar-nav text-on-surface-variant py-stack-md">
        ...
        <a class="hover:text-primary transition-colors" href="<?php echo e(route('retur.show', $retur)); ?>"><?php echo e($retur->kd_retur); ?></a>
        <span class="material-symbols-outlined text-[14px]">chevron_right</span>
    ...
    <form method="POST" action="<?php echo e(route('retur.update', $retur)); ?>" enctype="multipart/form-data" id="formEditRetur">
        <?php echo csrf_field(); ?>
        ...
        <div class="bg-surface-container-lowest rounded-xl shadow-sm border border-surface-container-high overflow-hidden flex flex-col">
            ...
            <div class="bg-inverse-surface text-inverse-on-surface p-container-padding flex flex-col gap-1">
                ...
                <h1 class="font-headline-md text-headline-md text-inverse-on-surface leading-tight mt-1"><?php echo e($retur->kd_retur); ?></h1>
            ...
            <div class="p-container-padding flex flex-col gap-stack-md">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <textarea name="catatan_retur" rows="2" class="bg-surface-container-low px-stack-md py-2 rounded-lg font-body-sm text-body-sm text-on-surface placeholder:text-outline focus:outline-none resize-none"><?php echo e($retur->catatan_retur); ?></textarea>
                </div>

                <div class="flex flex-col gap-stack-sm" id="itemCardsContainer">
                    <?php $__currentLoopData = $retur->details; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $detail): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="bg-surface-container-low p-stack-md rounded-xl border-2 border-primary/40 shadow-sm flex flex-col gap-stack-sm item-card">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-2 pb-2 border-b">
                                <div class="flex items-start gap-stack-sm">
                                    ...
                                    <input type="hidden" name="items[<?php echo e($index); ?>][id_retur_detail]" value="<?php echo e($detail->id_retur_detail); ?>" class="disableable">
                                    <div class="flex flex-col">
                                        <span class="font-label-bold text-body-sm text-on-surface"><?php echo e($detail->barang?->nm_master_barang); ?></span>
                                        <span class="text-[11px] text-outline">Qty Reject QC: <strong class="text-error"><?php echo e($detail->qty_reject_qc); ?> Unit</strong></span>
                                    </div>
                                </div>
                                <div class="flex items-center gap-stack-sm self-end sm:self-auto">
                                    ...
                                    <input type="number" min="1" max="<?php echo e($detail->qty_reject_qc); ?>" value="<?php echo e($detail->qty_diretur); ?>"
                                        name="items[<?php echo e($index); ?>][qty_diretur]"
                            ...
                            <div class="flex flex-col gap-1.5 pt-1">
                                ...
                                <div class="flex flex-wrap gap-1.5">
                                    <?php $chosenAlasan = $detail->alasan->pluck('id_alasan_retur')->all(); ?>
                                    <?php $__currentLoopData = $alasanList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $alasan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <?php $checked = in_array($alasan->id_alasan_retur, $chosenAlasan, true); ?>
                                        <label class="alasan-chip inline-flex items-center gap-1 px-2.5 py-1 rounded-full font-label-bold text-[11px] cursor-pointer transition-colors <?php echo e($checked ? 'bg-error-container text-on-error-container shadow-sm' : 'bg-surface-container text-on-surface-variant hover:bg-surface-container-high'); ?>">
                                            <input type="checkbox" class="sr-only alasan-checkbox disableable" name="items[<?php echo e($index); ?>][alasan_ids][]" value="<?php echo e($alasan->id_alasan_retur); ?>" <?php echo e($checked ? 'checked' : ''); ?>>
                                            <span class="material-symbols-outlined text-[13px] <?php echo e($checked ? '' : 'text-outline'); ?>"><?php echo e($checked ? 'check_circle' : 'add'); ?></span>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-stack-sm mt-2">
                                    <input type="text" name="items[<?php echo e($index); ?>][catatan_detail]" value="<?php echo e($detail->catatan_detail); ?>"
                                    ...
                                    <div class="flex flex-col gap-1.5">
                                        <?php if($detail->fotos->isNotEmpty()): ?>
                                            <div class="flex flex-wrap gap-1.5">
                                                <?php $__currentLoopData = $detail->fotos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $foto): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <a href="<?php echo e($foto->url); ?>" target="_blank" class="block w-12 h-12 rounded-lg overflow-hidden border border-surface-container-high shrink-0">
                                                        <img src="<?php echo e($foto->url); ?>" class="w-full h-full object-cover">
                                                    </a>
                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                            </div>
                                        <?php endif; ?>
                                        <label class="disableable inline-flex items-center gap-1 px-2 py-1.5 rounded-lg bg-surface-container hover:bg-surface-container-high text-on-surface text-[11px] font-label-bold transition-colors w-full justify-center cursor-pointer">
                                            ...
                                            <input type="file" name="items[<?php echo e($index); ?>][fotos][]" multiple accept="image/*" class="hidden">
                                        </label>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="bg-surface-container-low p-container-padding flex flex-col sm:flex-row items-center justify-between gap-stack-md border-t border-surface-container-high">
                ...
                <div class="flex items-center gap-stack-sm w-full sm:w-auto justify-end">
                    <a href="<?php echo e(route('retur.show', $retur)); ?>" class="px-container-padding py-2 rounded-lg bg-surface-container hover:bg-surface-container-high text-on-surface font-label-bold text-body-sm transition-colors">
                        Batal
                    </a>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\ProjectPDAM\laragon-6.0-minimal\www\WareHouse\resources\views/retur/edit.blade.php ENDPATH**/ ?>