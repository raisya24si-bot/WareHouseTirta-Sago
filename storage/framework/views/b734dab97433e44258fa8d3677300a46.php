...
<?php endif; ?></td><td class="px-5 py-4 text-right"><div class="inline-flex gap-1"><button type="button" onclick="editRow(<?php echo e($r->id_row); ?>,<?php echo \Illuminate\Support\Js::from($r->nm_row)->toHtml() ?>,<?php echo e($r->fk_rak); ?>,<?php echo \Illuminate\Support\Js::from($r->status_row)->toHtml() ?>)" class="p-1 text-outline hover:text-primary"><span class="material-symbols-outlined">edit</span></button><form method="POST" action="<?php echo e(route('master-row.destroy',$r)); ?>" onsubmit="return confirm('Hapus row ini?')"><?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?><button class="p-1 text-outline hover:text-error"><span class="material-symbols-outlined">delete</span></button></form></div></td></tr><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?><tr><td colspan="6" class="px-5 py-12 text-center text-on-surface-variant">Belum ada data row.</td></tr><?php endif; ?></tbody></table></div><?php /**PATH D:\ProjectPDAM\laragon-6.0-minimal\www\MasterData_REBUILT_CLEAN_FIXED\MasterData\resources\views/components/master/gudang/row-table.blade.php ENDPATH**/ ?>