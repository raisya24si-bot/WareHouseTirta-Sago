...
@section('title', 'Penerimaan Barang Pengganti Retur - Warehouse Tirta Sago')
@section('breadcrumb', 'Penerimaan Barang Pengganti Retur')
...
<div class="flex flex-col w-full pb-container-padding">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-stack-md py-stack-md">
        ...
        <div class="flex items-center gap-stack-sm shrink-0 self-start md:self-auto">
            <a href="{{ route('retur.index') }}" class="inline-flex items-center gap-stack-sm px-container-padding py-2 rounded-lg bg-surface-container hover:bg-surface-container-high text-on-surface font-label-bold text-body-sm transition-all shadow-sm">
        ...
        <div class="bg-surface-container-lowest p-stack-md rounded-xl shadow-sm flex flex-col justify-between">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <div class="flex items-baseline gap-stack-sm mt-stack-sm">
                        <span class="font-stat-number text-stat-number text-on-surface tracking-tight">{{ $summary['menunggu_kedatangan'] }}</span>
            ...
            <div class="mt-stack-md pt-stack-sm text-on-surface-variant font-body-sm text-[12px]">
                <span class="inline-flex items-center gap-1 text-tertiary font-label-bold">
                    ...
                    {{ $summary['unit_dalam_perjalanan'] }} Unit Barang Pengganti dlm Perjalanan
                </span>
            </div>
        </div>
        ...
        <div class="bg-surface-container-lowest p-stack-md rounded-xl shadow-sm flex flex-col justify-between">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <div class="flex items-baseline gap-stack-sm mt-stack-sm">
                        <span class="font-stat-number text-stat-number text-on-surface tracking-tight">{{ $summary['proses_qc'] }}</span>
        ...
        <div class="bg-surface-container-lowest p-stack-md rounded-xl shadow-sm flex flex-col justify-between">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <div class="flex items-baseline gap-stack-sm mt-stack-sm">
                        <span class="font-stat-number text-stat-number text-on-surface tracking-tight">{{ $summary['selesai_bulan_ini'] }}</span>
            ...
            <div class="mt-stack-md pt-stack-sm text-on-surface-variant font-body-sm text-[12px]">
                <span class="inline-flex items-center gap-1 text-on-surface font-label-bold">
                    <span class="material-symbols-outlined text-[14px]">check</span>
                    {{ $summary['unit_kembali_stok_bulan_ini'] }} Unit Barang Kembali ke Stok Aktif
                </span>
            </div>
        </div>
    </div>

    <form method="GET" action="{{ route('penerimaan-retur.index') }}" id="filterForm">
        ...
        <div class="flex items-center justify-between overflow-x-auto gap-stack-md bg-surface-container-lowest px-container-padding py-stack-sm rounded-t-xl shadow-sm">
            <div class="flex items-center gap-stack-sm shrink-0">
                ...
                <a href="{{ route('penerimaan-retur.index', array_merge(request()->except(['status', 'page']), ['status' => ''])) }}"
                ...
                <a href="{{ route('penerimaan-retur.index', array_merge(request()->except(['status', 'page']), ['status' => 'proses'])) }}"
                ...
                <a href="{{ route('penerimaan-retur.index', array_merge(request()->except(['status', 'page']), ['status' => 'selesai'])) }}"
        ...
        <div class="bg-surface-container-low px-container-padding py-stack-md shadow-sm flex flex-col lg:flex-row items-center justify-between gap-stack-md">
            <div class="relative w-full lg:w-96">
                ...
                <input name="search" value="{{ request('search') }}" class="w-full bg-surface-container-lowest pl-9 pr-stack-md py-2 rounded-lg font-body-sm text-body-sm text-on-surface placeholder:text-outline focus:outline-none shadow-sm" placeholder="Cari No. Penerimaan, No. Retur, No. GRN, Supplier..." type="text">
            </div>
            <div class="flex flex-wrap items-center gap-stack-sm w-full lg:w-auto">
                ...
                <a href="{{ route('penerimaan-retur.index') }}" class="p-2 rounded-lg bg-surface-container hover:bg-surface-container-high text-on-surface transition-colors" title="Reset Filter">
    ...
    <div class="flex flex-col bg-surface-container-lowest rounded-b-xl shadow-sm overflow-hidden">
                        ...
                        <tr class="hover:bg-surface-container-low transition-colors">
                            <td class="p-stack-md align-top">
                                <div class="flex flex-col gap-0.5">
                                    <span class="font-label-bold text-primary font-mono text-[13px]">{{ $penerimaan->kd_penerimaan_retur }}</span>
                                    <span class="text-[12px] text-on-surface-variant">{{ $penerimaan->created_at->translatedFormat('d M Y, H:i') }}</span>
                                </div>
                            </td>
                            <td class="p-stack-md align-top">
                                <div class="flex flex-col gap-0.5">
                                    <span class="font-mono font-bold text-on-surface text-[12px]">{{ $penerimaan->returBarang?->kd_retur }}</span>
                                    <span class="text-[11px] text-outline font-mono">Asal: {{ $penerimaan->returBarang?->penerimaanBarang?->kd_penerimaan }}</span>
                                </div>
                            </td>
                            <td class="p-stack-md align-top">
                                <div class="flex flex-col gap-0.5">
                                    <span class="font-mono font-bold text-on-surface text-[12px]">{{ $penerimaan->no_sj_supplier }}</span>
                                    @if($penerimaan->returBarang?->no_resi_pengiriman)
                                        <span class="text-[11px] text-outline">Resi: {{ $penerimaan->returBarang->no_resi_pengiriman }}</span>
                                    @endif
                                </div>
                            </td>
                            <td class="p-stack-md align-top">
                                <div class="flex flex-col">
                                    <span class="font-label-bold text-on-surface">{{ $penerimaan->supplier?->nm_master_supplier ?? '-' }}</span>
                                    <span class="text-[11px] text-outline">{{ $penerimaan->supplier?->kontak_supplier ?? '-' }}</span>
                                </div>
                            </td>
                            <td class="p-stack-md align-top text-center">
                                <div class="flex flex-col items-center">
                                    <span class="font-label-bold text-primary text-[15px]">{{ $penerimaan->totalQtyTiba() }} Unit</span>
                                    <span class="text-[11px] text-outline">{{ $penerimaan->details->count() }} SKU</span>
                                </div>
                            </td>
                            <td class="p-stack-md align-top">
                                <span class="font-mono text-[12px] text-on-surface">{{ $binList->implode(', ') ?: '-' }}</span>
                            </td>
                            <td class="p-stack-md align-top">
                            ...
                            <td class="p-stack-md align-top text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('penerimaan-retur.show', $penerimaan) }}" class="p-1.5 rounded-lg bg-surface-container hover:bg-surface-container-high text-primary transition-colors inline-flex" title="Lihat Detail">
        ...
        <div class="p-container-padding bg-surface-container-lowest flex flex-col sm:flex-row items-center justify-between gap-stack-md border-t">
            <span class="font-body-sm text-[13px] text-outline">
                Menampilkan <strong class="text-on-surface">{{ $penerimaans->firstItem() ?? 0 }} - {{ $penerimaans->lastItem() ?? 0 }}</strong> dari <strong class="text-on-surface">{{ $penerimaans->total() }}</strong> data penerimaan pengganti retur
            </span>
            {{ $penerimaans->onEachSide(1)->links() }}
        </div>
    </div>
</div>

{{-- Modal: Konfirmasi Penerimaan Barang Pengganti --}}
...
<div class="bg-surface-container-lowest rounded-xl shadow-md w-full max-w-5xl my-stack-md overflow-hidden flex flex-col border border-surface-container-high">
    ...
    <form method="POST" action="{{ route('penerimaan-retur.store') }}" id="formTerimaRetur">
        @csrf
        ...
        <div class="p-container-padding overflow-y-auto max-h-[70vh] flex flex-col gap-stack-md">
            ...
            {{-- Section 1: Ringkasan Pengiriman --}}
            <div class="bg-surface-container-low p-stack-md rounded-xl border border-surface-container flex flex-col gap-stack-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-stack-md">
                    <div>
                        ...
                        <select name="fk_retur" id="returSelector" required class="w-full bg-surface-container-lowest border border-surface-container-highest rounded-lg p-2.5 font-body-sm text-on-surface focus:ring-1 focus:ring-primary focus:outline-none">
                            ...
                            @foreach($returEligible as $retur)
                                <option
                                    value="{{ $retur->id_retur }}"
                                    data-supplier="{{ $retur->supplier?->nm_master_supplier }}"
                                    data-kontak="{{ $retur->supplier?->kontak_supplier }}"
                                >
                                    {{ $retur->kd_retur }} ({{ $retur->supplier?->nm_master_supplier }} - {{ $retur->totalItemReject() }} Unit Reject)
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-stack-sm">
                    ...
                    <div class="bg-surface-container-lowest p-3 rounded-lg border border-surface-container-high">
                        ...
                        <input type="datetime-local" name="waktu_tiba_dock" value="{{ now()->format('Y-m-d\TH:i') }}" class="w-full bg-transparent text-[12px] font-label-bold text-on-surface focus:outline-none">
                    ...
                    <div class="bg-surface-container-lowest p-3 rounded-lg border border-surface-container-high">
                        ...
                        <p class="font-label-bold text-on-surface text-[13px] mt-0.5">{{ auth()->user()->name ?? 'Petugas Inbound' }}</p>
                        <p class="text-[11px] text-outline">{{ auth()->user()->jabatan ?? '' }}</p>
                    </div>
                </div>
            </div>

            {{-- Section 2: Verifikasi Fisik & Alokasi Bin --}}
...
{{-- Data bin buat dirender JS di tiap baris item --}}
