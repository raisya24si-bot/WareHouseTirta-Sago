    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-stack-md mt-base">
        ...
        <div class="flex items-center gap-stack-sm shrink-0">
            <div class="hidden xl:flex flex-col items-end px-3 py-1.5 rounded-lg bg-surface-container-low text-right">
                ...
                <span class="text-[12px] font-label-bold text-on-surface">SAP MM • {{ now()->format('H:i') }} WIB</span>
            </div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-gutter mt-stack-md">
    <div class="flex flex-col justify-between p-stack-md rounded-xl bg-tertiary-fixed text-on-tertiary-fixed shadow-sm relative overflow-hidden group">
        ...
        <div class="mt-4 z-10">
            <div class="font-stat-number text-stat-number text-on-tertiary-fixed">{{ number_format($summary['menunggu_verifikasi']) }}</div>
    ...
    <div class="flex flex-col justify-between p-stack-md rounded-xl bg-secondary-fixed text-on-secondary-fixed shadow-sm relative overflow-hidden group">
        ...
        <div class="mt-4 z-10">
            <div class="font-stat-number text-stat-number text-on-secondary-fixed">{{ number_format($summary['dalam_alokasi']) }}</div>
    ...
    <div class="flex flex-col justify-between p-stack-md rounded-xl bg-primary-fixed text-on-primary-fixed shadow-sm relative overflow-hidden group">
        ...
        <div class="mt-4 z-10">
            <div class="font-stat-number text-stat-number text-on-primary-fixed">{{ number_format($summary['selesai']) }}</div>
    ...
    <div class="flex flex-col justify-between p-stack-md rounded-xl bg-surface-container text-on-surface shadow-sm relative overflow-hidden group">
        ...
        <div class="mt-4 z-10 flex items-baseline gap-2">
            <div class="font-stat-number text-stat-number text-on-surface">{{ number_format($summary['total_barang']) }}</div>
        ...
        <div class="flex flex-col gap-stack-md">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                <div class="flex flex-wrap items-center gap-1.5 p-1 rounded-xl bg-surface-container-low w-fit" role="tablist">
                    <a href="{{ route('penerimaan.index') }}" class="px-3.5 py-2 rounded-lg {{ !request('status') ? 'bg-surface-container-lowest font-label-bold text-primary shadow-sm' : 'font-sidebar-nav text-on-surface-variant hover:text-on-surface' }} transition-colors text-[13px] whitespace-nowrap">
                        Semua ({{ number_format($tabCounts['all']) }})
                    </a>
                    <a href="{{ route('penerimaan.index', ['status' => 'menunggu']) }}" class="px-3.5 py-2 rounded-lg {{ request('status') === 'menunggu' ? 'bg-surface-container-lowest font-label-bold text-primary shadow-sm' : 'font-sidebar-nav text-on-surface-variant hover:text-on-surface' }} transition-colors flex items-center gap-1.5 text-[13px] whitespace-nowrap">
                        <span class="w-2 h-2 rounded-full bg-tertiary-container"></span>
                        Menunggu Verifikasi ({{ number_format($tabCounts['menunggu']) }})
                    </a>
                    <a href="{{ route('penerimaan.index', ['status' => 'alokasi']) }}" class="px-3.5 py-2 rounded-lg {{ request('status') === 'alokasi' ? 'bg-surface-container-lowest font-label-bold text-primary shadow-sm' : 'font-sidebar-nav text-on-surface-variant hover:text-on-surface' }} transition-colors flex items-center gap-1.5 text-[13px] whitespace-nowrap">
                        <span class="w-2 h-2 rounded-full bg-secondary"></span>
                        Dalam Alokasi ({{ number_format($tabCounts['alokasi']) }})
                    </a>
                    <a href="{{ route('penerimaan.index', ['status' => 'APPROVED']) }}" class="px-3.5 py-2 rounded-lg {{ request('status') === 'APPROVED' ? 'bg-surface-container-lowest font-label-bold text-primary shadow-sm' : 'font-sidebar-nav text-on-surface-variant hover:text-on-surface' }} transition-colors flex items-center gap-1.5 text-[13px] whitespace-nowrap">
                        <span class="w-2 h-2 rounded-full bg-primary"></span>
                        Selesai / Approved ({{ number_format($tabCounts['selesai']) }})
                    </a>
                </div>

                <div class="flex items-center gap-2 self-start lg:self-auto shrink-0">
                    <a href="{{ route('penerimaan.export', request()->query()) }}" class="inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-lg bg-surface-container text-on-surface hover:bg-surface-container-high transition-colors font-label-bold text-[13px] whitespace-nowrap">
                    ...
                    <button type="button" id="advancedFilterBtn" class="relative inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-lg border border-outline-variant text-on-surface hover:bg-surface-container-low transition-colors font-label-bold text-[13px] whitespace-nowrap">
                        ...
                        Filter Lanjutan
                        @if(request()->hasAny(['gudang', 'per_page']) || in_array(request('status'), ['DRAFT', 'REJECTED'], true))
            ...
            <form method="GET" action="{{ route('penerimaan.index') }}" id="filterForm" class="flex flex-col gap-2.5">
                <div class="flex flex-col lg:flex-row lg:items-center gap-2.5">
                    <div class="relative flex-1 min-w-0">
                        ...
                        <input id="penerimaanSearch" name="search" value="{{ request('search') }}" class="w-full h-11 bg-surface-container-low pl-11 pr-3.5 rounded-lg font-body-sm text-[13px] text-on-surface placeholder:text-outline focus:outline-none focus:bg-surface-container-lowest focus:ring-2 focus:ring-primary/20 transition-all" placeholder="Cari No. Penerimaan, No. PO, Supplier, atau No. Invoice..." type="text">
                    </div>
                    ...
                    <div class="relative w-full lg:w-[270px] shrink-0" id="dateRangeWrap">
                        <button type="button" id="dateRangeButton" class="w-full h-11 flex items-center gap-2.5 px-3.5 rounded-lg bg-surface-container-low border border-transparent hover:border-outline-variant focus:outline-none focus:ring-2 focus:ring-primary/20 text-left transition-all">
                            ...
                            <span id="dateRangeLabel" class="flex-1 truncate text-[13px] text-on-surface">{{ request('date_from') || request('date_to') ? ((request('date_from') ? \Carbon\Carbon::parse(request('date_from'))->format('d M Y') : 'Semua tanggal') . ' - ' . (request('date_to') ? \Carbon\Carbon::parse(request('date_to'))->format('d M Y') : 'Sekarang')) : 'Pilih rentang tanggal' }}</span>
                        ...
                        <div id="dateRangePopover" class="hidden absolute left-0 top-full z-30 mt-2 w-full min-w-[330px] rounded-xl border border-outline-variant bg-surface-container-lowest p-4 shadow-xl">
                            ...
                            <div class="grid grid-cols-2 gap-2.5">
                                <div>
                                    ...
                                    <input id="dateFrom" name="date_from" value="{{ request('date_from') }}" type="date" class="w-full h-10 rounded-lg border border-outline-variant bg-surface-container-low px-2.5 text-[12px] text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20">
                                </div>
                                <div>
                                    <label for="dateTo" class="mb-1 block text-[11px] font-label-bold text-on-surface-variant">Sampai</label>
                                    <input id="dateTo" name="date_to" value="{{ request('date_to') }}" type="date" class="w-full h-10 rounded-lg border border-outline-variant bg-surface-container-low px-2.5 text-[12px] text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20">
                                </div>
                            </div>
                    ...
                    <div class="w-full lg:w-[230px] shrink-0">
                        <select id="gudangFilter" name="gudang" onchange="this.form.submit()" class="w-full h-11 bg-surface-container-low border border-transparent px-3.5 rounded-lg font-body-sm text-[13px] text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20 cursor-pointer">
                            ...
                            @foreach($gudangs as $gudang)
                                <option value="{{ $gudang->id_gudang }}" @selected((string) request('gudang') === (string) $gudang->id_gudang)>{{ $gudang->nm_gudang }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-stack-sm">
                        ...
                        <div>
                            ...
                            <select id="perPageFilter" name="per_page" class="w-full h-10 bg-surface-container-lowest border border-outline-variant px-3 rounded-lg font-body-sm text-[13px] text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20 cursor-pointer">
                                @foreach(['10', '20', '30', '50', 'all'] as $option)
                                    <option value="{{ $option }}" @selected($perPage === $option)>{{ $option === 'all' ? 'Semua' : $option }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="flex items-end gap-2">
                            ...
                            @if(request()->hasAny(['search', 'status', 'date_from', 'date_to', 'gudang', 'per_page']))
                                <a href="{{ route('penerimaan.index') }}" class="h-10 px-3 rounded-lg border border-outline-variant text-on-surface-variant hover:bg-surface-container-lowest flex items-center justify-center transition-colors" title="Reset semua filter">
                    ...
                    <tr class="hover:bg-surface-container-low/60 transition-colors">
                        <td class="py-3.5 px-stack-md">
                            <a href="{{ route('penerimaan.verifikasi', $penerimaan) }}" class="font-label-bold text-primary flex items-center gap-2 hover:underline">
                                <span class="material-symbols-outlined text-[16px] text-outline">{{ $kodeStatus === 'DRAFT' ? 'draft' : 'description' }}</span>
                                {{ $penerimaan->kd_penerimaan }}
                            </a>
                        </td>
                        <td class="py-3.5 px-stack-md text-on-surface-variant">
                            {{ $penerimaan->tgl_penerimaan_barang?->format('d M Y') ?? '-' }}
                            <span class="text-[11px] text-outline block">{{ $penerimaan->created_at?->format('H:i') ?? '-' }} WIB</span>
                        </td>
                        <td class="py-3.5 px-stack-md font-medium text-on-surface">{{ $penerimaan->po?->kd_po ?? '-' }}</td>
                        <td class="py-3.5 px-stack-md font-sidebar-nav text-[12px] text-on-surface-variant">{{ $penerimaan->no_sjinv_supplier ?: '-' }}</td>
                        <td class="py-3.5 px-stack-md">
                            <div class="font-label-bold text-on-surface">{{ $supplierName }}</div>
                            <div class="text-[11px] text-on-surface-variant">{{ $penerimaan->desc_penerimaan_barang ?: 'Detail penerimaan barang dari PO' }}</div>
                        </td>
                        <td class="py-3.5 px-stack-md text-right">
                            <span class="font-label-bold text-on-surface">{{ $penerimaan->totalSku() }} SKU</span>
                            <span class="text-[11px] text-on-surface-variant block">{{ number_format($penerimaan->totalQtyRequest()) }} {{ $unitName }}</span>
                        </td>
                        <td class="py-3.5 px-stack-md">
                            @php $pic = $penerimaan->submittedBy?->name ?? '-'; @endphp
                            <div class="flex items-center gap-1.5">
                                <div class="w-6 h-6 rounded-full bg-secondary-fixed text-on-secondary-fixed flex items-center justify-center text-[10px] font-bold">{{ $pic !== '-' ? strtoupper(substr($pic, 0, 2)) : '--' }}</div>
                        ...
                        <td class="py-3.5 px-stack-md">
                            @if($isPending)
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-tertiary-fixed text-on-tertiary-fixed text-[11px] font-label-bold">
                                    <span class="w-1.5 h-1.5 rounded-full bg-tertiary-container"></span> {{ $statusLabel }}
                                </span>
                            @elseif($kodeStatus === 'APPROVED')
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-primary-fixed text-on-primary-fixed text-[11px] font-label-bold">
                                    <span class="w-1.5 h-1.5 rounded-full bg-primary"></span> {{ $statusLabel }}
                                </span>
                            @elseif($kodeStatus === 'REJECTED')
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-error-container text-on-error-container text-[11px] font-label-bold">
                                    <span class="w-1.5 h-1.5 rounded-full bg-error"></span> {{ $statusLabel }}
                                </span>
                            @elseif($hasLocation)
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-secondary-fixed text-on-secondary-fixed text-[11px] font-label-bold">
                                    <span class="w-1.5 h-1.5 rounded-full bg-secondary"></span> {{ $statusLabel }} • Alokasi
                                </span>
                            @else
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-surface-container-highest text-on-surface-variant text-[11px] font-label-bold">
                                    <span class="w-1.5 h-1.5 rounded-full bg-outline"></span> {{ $statusLabel }}
                                </span>
                            @endif
                        </td>
                        <td class="py-3.5 px-stack-md text-center">
                            @if($kodeStatus === 'DRAFT' || $kodeStatus === 'REJECTED')
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-gutter mt-stack-md">
    ...
    <div class="self-start p-container-padding rounded-xl bg-surface-container-low shadow-sm flex flex-col">
        ...
        <div class="mt-3 flex items-center gap-3">
            <div class="relative shrink-0 w-[64px] h-[64px]">
                <svg viewBox="0 0 64 64" class="w-full h-full -rotate-90">
                    <circle cx="32" cy="32" r="{{ $ringRadius }}" fill="none" stroke-width="6" class="stroke-surface-container-highest"></circle>
                    <circle
                        cx="32" cy="32" r="{{ $ringRadius }}" fill="none" stroke-width="6"
                ...
                <div class="absolute inset-0 flex items-center justify-center">
                    <span class="font-headline-md text-[13px] font-bold text-on-surface">{{ number_format($accuracyPct, 1) }}%</span>
                </div>
            </div>
            ...
            <div class="flex-1 min-w-0 flex flex-col gap-1 text-[11px] font-body-sm">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <span class="font-label-bold text-on-surface">{{ number_format($summary['item_sesuai']) }} Unit</span>
                </div>
                <div class="flex items-center justify-between gap-3">
                    <span class="text-on-surface-variant">Discrepancy / Rusak:</span>
                    <span class="font-label-bold text-error">{{ number_format($summary['item_discrepancy']) }} Unit ({{ number_format($summary['discrepancy_percentage'], 1) }}%)</span>
                </div>
                <div class="flex items-center justify-between gap-3">
                    <span class="text-on-surface-variant">Rata-rata Waktu Verifikasi:</span>
                    <span class="font-label-bold text-on-surface">{{ number_format($summary['avg_verifikasi_minutes']) }} Menit</span>
                </div>
            </div>
        </div>
        ...
        <div class="mt-3 pt-2 border-t border-outline-variant flex items-center justify-between text-[11px]">
            ...
            <a href="{{ route('penerimaan.laporan-akurasi', request()->query()) }}" class="font-label-bold text-primary hover:underline">Unduh PDF</a>
        </div>
    </div>
</div>
...
<div class="fixed inset-0 z-50 hidden bg-on-surface/40 backdrop-blur-[2px]" id="goodsReceiptModal">
    <div class="min-h-full flex items-center justify-center p-4">
        <div class="w-full max-w-3xl rounded-2xl bg-surface-container-lowest shadow-2xl overflow-hidden">
            ...
            <form method="POST" action="{{ route('penerimaan.store') }}" id="goodsReceiptForm">
                @csrf
                <div class="p-container-padding overflow-y-auto max-h-[70vh] flex flex-col gap-stack-md">
                    <div class="flex flex-col gap-1">
                        ...
                        <div class="relative">
                            <select name="fk_po" required class="w-full bg-surface-container-low text-on-surface px-stack-md py-2.5 rounded-lg font-body-sm text-body-sm focus:outline-none focus:ring-2 focus:ring-primary appearance-none cursor-pointer" id="poSelector">
                                ...
                                @foreach($poTerbuka as $po)
                                    <option value="{{ $po->id_po }}" @selected(old('fk_po') == $po->id_po)>
                                        {{ $po->kd_po }} — {{ $po->supplier?->nm_master_supplier ?? '-' }} ({{ $po->details->count() }} Item)
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-stack-md">
                        <div class="flex flex-col gap-1">
                            ...
                            <input name="no_sjinv_supplier" value="{{ old('no_sjinv_supplier') }}" class="bg-surface-container-low px-stack-md py-2.5 rounded-lg font-body-sm text-body-sm text-on-surface placeholder:text-outline focus:outline-none focus:ring-2 focus:ring-primary" id="invNumber" placeholder="Contoh: INV-2026-SMJ-9912" type="text">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <input name="tgl_penerimaan_barang" value="{{ old('tgl_penerimaan_barang', now()->format('Y-m-d')) }}" required class="bg-surface-container-low px-stack-md py-2.5 rounded-lg font-body-sm text-body-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-primary" id="receivedDate" type="date">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <div class="flex items-center gap-3 bg-surface-container-low px-stack-md py-2.5 rounded-lg">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="font-label-bold text-[13px] text-on-surface">{{ auth()->user()?->name ?? 'User' }}</div>
...
@if(session('success'))
    <div class="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-stack-md py-3 rounded-xl bg-inverse-surface text-inverse-on-surface shadow-2xl" id="toastSuccess">
        ...
        <div class="flex flex-col">
            ...
            <span class="font-body-sm text-[11px] text-inverse-on-surface/80">{{ session('success') }}</span>
        </div>
    </div>
@endif
...
(function () {
    ...
    @if(request()->hasAny(['per_page']) || in_array(request('status'), ['DRAFT', 'REJECTED'], true))
