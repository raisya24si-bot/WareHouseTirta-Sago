...
@section('title', 'Approval ' . $config['label'] . ' (GRN) - Warehouse Tirta Sago')
@section('breadcrumb', 'Penerimaan Barang PO / Approval ' . $config['label'])
...
<div class="flex flex-col gap-base pt-container-padding">
    ...
    <nav class="flex items-center gap-stack-sm text-body-sm text-on-surface-variant font-body-sm">
        ...
        <span class="material-symbols-outlined text-[14px] text-outline">chevron_right</span>
        <span class="font-label-bold text-on-surface">Approval {{ $config['label'] }}</span>
    </nav>
        ...
        <div>
            <div class="flex items-center gap-stack-sm mb-1">
                <span class="px-2 py-0.5 rounded-full bg-primary-fixed text-on-primary-fixed text-[11px] font-bold tracking-wider uppercase">
                    {{ $isFinalLevel ? 'Final Approval' : 'Tingkat ' . $config['order'] }}
                </span>
                <span class="text-[12px] text-outline font-sidebar-nav">{{ $config['label'] }}</span>
            </div>
            <h1 class="font-display-lg text-display-lg text-on-surface tracking-tight">Persetujuan Penerimaan Barang (GRN) &mdash; {{ $config['label'] }}</h1>
        </div>
    </div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-gutter mt-stack-md">
    ...
    <div class="flex flex-col justify-between p-stack-md rounded-xl bg-primary-fixed text-on-primary-fixed shadow-sm relative overflow-hidden group">
        ...
        <div class="mt-4 z-10">
            ...
            <div class="text-[12px] font-body-sm text-on-primary-fixed-variant mt-1 font-medium">{{ $isFinalLevel ? 'Stok sudah masuk ke gudang' : 'Diteruskan ke tingkat berikutnya' }}</div>
        </div>
    </div>
...
<div class="mt-stack-md rounded-xl bg-surface-container-lowest shadow-sm overflow-hidden">
    ...
    <div class="flex items-center gap-1 px-container-padding pt-3 border-b border-outline-variant">
        ...
        @foreach($tabs as $tabKey => $tabInfo)
            ...
            <a
                href="{{ route('penerimaan.approval.index', [$level, 'tab' => $tabKey]) }}"
    ...
    <div class="p-container-padding border-b border-outline-variant">
        ...
        <form method="GET" action="{{ route('penerimaan.approval.index', $level) }}" class="flex items-center gap-stack-sm">
            ...
            @if(request()->hasAny(['search', 'per_page']))
                <a href="{{ route('penerimaan.approval.index', [$level, 'tab' => $tab]) }}" class="h-11 px-3 rounded-lg border border-outline-variant text-on-surface-variant hover:bg-surface-container-lowest flex items-center justify-center transition-colors" title="Reset">
        ...
        <table class="w-full text-left font-body-sm text-body-sm text-on-surface">
            ...
            <thead class="bg-surface-container-low font-label-bold text-label-bold text-on-surface-variant text-[12px] uppercase tracking-wider">
                <tr>
                    ...
                    <th class="py-3 px-stack-md">{{ $tab === 'menunggu' ? 'Kondisi' : 'Keputusan ' . $config['label'] }}</th>
                        ...
                        <td class="py-3.5 px-stack-md">
                            ...
                            @elseif($tab === 'disetujui')
                                ...
                                <div class="text-[11px] text-on-surface-variant mt-1">
                                    {{ $actorName }} &bull;
                                    {{ $penerimaan->{$atField}?->translatedFormat('d M Y, H:i') ?? '-' }}
                                </div>
                            ...
                            @else
                                ...
                                <div class="text-[11px] text-on-surface-variant mt-1">
                                    {{ $penerimaan->rejectedBy?->name ?? '-' }} &bull;
                                    {{ $penerimaan->reject_at?->translatedFormat('d M Y, H:i') ?? '-' }}
                                </div>
                                @if($penerimaan->reject_note ?? $penerimaan->catatan_approval)
                                    <div class="text-[11px] text-red-600 italic mt-0.5">
                                        "{{ \Illuminate\Support\Str::limit($penerimaan->reject_note ?? $penerimaan->catatan_approval, 60) }}"
                                    </div>
                                @endif
