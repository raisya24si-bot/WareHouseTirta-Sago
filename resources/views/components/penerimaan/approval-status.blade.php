...
<div {{ $attributes->class(['approval-status-widget']) }}>
    ...
    <div class="flex items-start">
        ...
        @foreach(\App\Models\PenerimaanBarang::LEVELS as $slug => $stepConfig)
            ...
            @if($order < count(\App\Models\PenerimaanBarang::LEVELS))
    ...
    @if($withDetails && $penerimaan->isRejected() && $penerimaan->reject_note)
        ...
        <div class="mt-5 rounded-lg border border-red-200 bg-red-50 p-4">
            ...
            <div class="flex items-center gap-2 text-red-700">
                ...
                <span class="font-bold">
                    Ditolak oleh {{ $penerimaan->reject_level ? ucfirst(strtolower($penerimaan->reject_level)) : 'approver' }}
                    @if($penerimaan->rejectedBy)
                        ({{ $penerimaan->rejectedBy->name }})
                    @endif
                </span>
