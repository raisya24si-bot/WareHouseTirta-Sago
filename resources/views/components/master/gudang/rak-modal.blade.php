...
<div><label class="mb-1 block font-semibold">Gudang *</label><select id="rak-gudang" name="fk_gudang" required class="w-full rounded-md border border-outline-variant px-3 py-2">@foreach($gudangs as $g)<option value="{{ $g->id_gudang }}">{{ $g->kd_gudang }} - {{ $g->nm_gudang }}</option>@endforeach</select></div>
...
function openRakModal(){
    ...
    document.getElementById('rak-form').action='{{ route('master-rak.store') }}';
...
function editRak(id,status){
    ...
    document.getElementById('rak-form').action='{{ url('/master-rak') }}/'+id;
